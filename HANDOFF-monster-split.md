...
Cluster **C (restore/reattach)** was extracted on branch **`split-monster-acf`** (off `origin/main`), commit **`b9147b9`**, pushed. NOT a rewrite — a disciplined MOVE: bodies were kept byte-identical except for cross-file seam-call delegation **plus two deliberate, documented tab-scoping fixes bundled into the extraction** (see "Bonus finds"): `setFormLocked`'s `#gradingForm` → `[id="gradingForm"]` selector, and `reattachHighlightsHandlers`'s `TabStore.paneForTab(...)` checkbox resolution + 2-arg `setupRemoveAllCheckbox` call (the cross-tab bleed fix). So "byte-identical" holds for the *move mechanics*, but this was not a strictly behavior-preserving move — anyone bisecting a multi-tab regression to these commits should expect those two intended changes. This proved the approach F should follow.
