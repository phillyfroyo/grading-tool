...
- **Remaining un-tab-scoped remove-all lookups (latent cross-tab risk).** The 2026-06-27 `cleanup-june-2026` work tab-scoped the remove-all STATE (localStorage key via `removeAllStorageKey`) and the MARKS tagging (`applyRemoveAllStateToMarks` / `syncAllRemoveAllStateToMarks` via `TabStore.queryInTab`), fixing the observed cross-tab leak (a remove-all checked in one tab struck out a same-index essay's highlights in another tab → blank PDF). It also centralized the contentId→checkbox-id derivation into `removeAllCheckboxId`. BUT a few sibling lookups still use document-wide queries and rely on running in active-tab render context:
  - `setupRemoveAllCheckbox` (`display-utils.js`) — `document.getElementById(\`${contentId}-remove-all\`)`.
  - `updateRemoveAllCheckboxState` (`display-utils.js`) — same un-scoped checkbox lookup.
  - `populateHighlightsContent` (`display-utils.js`, ~line 481) — `document.querySelector('.formatted-essay-content[data-essay-index="N"]')` un-scoped.
  - **Why not fixed now:** they work in practice (they run during the active tab's render), and re-surgering the fragile setup/populate path in the same session that just fixed the live bug carries more risk than the latent issue. **When touched:** scope each to its tab via `TabStore.queryInTab(tabId, …)` (thread tabId in), exactly as `applyRemoveAllStateToMarks` now does. These are the next instances of the same "index isn't unique across tabs" pattern — fix them if/when a cross-tab symptom is traced to them, or as part of a dedicated remove-all hardening pass.
