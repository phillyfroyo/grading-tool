...
function createManualCategoryButtons() {
    ...
    return window.CATEGORIES.getManualCategories().map(category => {
        ...
        return `<button class="category-btn" data-category="${category.id}" style="background: ${bgColor}; color: ${textColor}; border: 2px solid ${category.color}; padding: 8px 12px; border-radius: 20px; cursor: pointer; font-weight: bold; transition: all 0.2s; ${decoration}">${category.name}</button>`;
    }).join('\n        ');
}
